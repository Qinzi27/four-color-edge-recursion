"""Replay all frozen drawings with stage-aware initialization and optional levels.

The candidate receives geometry alone, makes one deterministic attempt, and is
checked independently. The v2 comparison is copied from hash-bound evidence;
...
from scripts.validate_level_sides_peer_full import source_hashes as previous_source_hashes
from scripts.validate_relation_frontier_full import failure_rank, require, validate_inventory
...
NEW_SOURCES = ("fourcolor/staged_levels.py", "scripts/validate_staged_levels.py",
               "scripts/validate_staged_levels_full.py", "docs/STAGED_LEVEL_RULES-2026-09-19.md")
...
def build_report(source_path, diagnostic_path, checkpoint_dir, workers=4, batch_size=25, limit=None, resume=False):
    ...
    require(previous["policy"] == BASELINE, "source is not the frozen v2 policy")
    ...
    require(previous["diagnostic_evidence"] == {"filename": diagnostic_path.name, "sha256": file_sha(diagnostic_path)},
            "diagnostic evidence differs from frozen v2 corpus")
    require(diagnostic["source_sha256"] == diagnostic["source_sha256_end"], "diagnostic code drifted")
    require(all(hashes[name] == value for name, value in diagnostic["source_sha256"].items()),
            "frozen diagnostic source changed")
    six_keys = {row["key"] for row in diagnostic["records"]}
    old_failure_keys = {row["key"] for row in rows if row["runs"][BASELINE]["status"] != "solved"}
    require(len(six_keys) == 6 and six_keys <= {row["key"] for row in rows}, "six-case coverage changed")
    require(len(old_failure_keys) == 1, "frozen v2 unique failure count changed")
    diagnostics = six_keys | old_failure_keys
    ...
    return {"schema_version": 1, "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "experiment": "Stage-aware initialization and optional mother levels: entire frozen inventory",
        ...
        "baseline_evidence": "Copied unchanged from hash-bound independently checked v2 full report; not rerun",
        ...
        "limits": ["Complete finite already-inspected corpus, not all plane maps or an unseen holdout.",
            "Only v3 is rerun; v2 outcomes and runtimes are archived comparisons.",
            "Every geometry prefix restarts independently; final success does not erase a failed prefix.",
            "All failed runs and the seven predeclared diagnostics retain full certificates.",
            "The six earlier diagnostics may have new trajectories; exact v2 reproduction is not required.",
            ...
            "Initialization orientation and tie conventions are deterministic proposals, not proved universally safe."]}
