"""Independently audit the frozen stage-initialization full-corpus experiment.
...
plus the seven old diagnostics is newly solved. This is NOT a second full solve.
...
BASELINE = "mother-level-peer-sides-v2"
...
def audit_checkpoints(report, folder):
    ...
    for field in ("source_evidence", "diagnostic_evidence", "source_sha256", "policy", "baseline",
...
def audit_geometries_and_samples(report, batch_size=200):
    ...
    for start in range(0, len(rows), batch_size):
        ...
        for row, exported in zip(batch, exports):
            ...
            if key in details:
                ...
                require(detail["previous_result"] == row["runs"][BASELINE], "detail archived v2 changed")
            ...
            if key in sample_keys:
                fresh = restart_staged_level_names(geometry)
    ...
    return {"geometries_reconstructed": len(geometry_records),
            ...
            "fresh_sample_replay": {"selection": "15 lexicographically smallest geometry keys plus all 7 predeclared diagnostics",
...
def main():
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "outputs/staged-levels-full-2026-09-19.json.gz")
    parser.add_argument("--summary", type=Path, default=ROOT / "outputs/staged-levels-full-summary-2026-09-19.json")
    ...
    require(report["source_evidence"]["sha256"] == SOURCE_SHA256, "source is not frozen v2 full corpus")
    ...
    six = {row["key"] for row in inputs["diagnostic_evidence"]["records"]}
    ...
    require(len(six) == 6 and len(old_failures) == 1 and not six.intersection(old_failures), "diagnostic inventory changed")
    diagnostics = six | old_failures
    require(report["diagnostic_keys"] == sorted(diagnostics) and report["diagnostics_checked"] == 7,
            "seven-diagnostic coverage changed")
    ...
    for row in rows:
        ...
        require(row["runs"][BASELINE] == prior["runs"][BASELINE], "archived v2 result changed")
    ...
    result = {"generated_at_utc": datetime.now(timezone.utc).isoformat(), "passed": True,
        ...
        "archived_v2_records_copied_exactly": 7069, "summary": summaries, "paired_comparisons": comparisons,
        "initialization_summary": {k: v for k, v in initialization.items() if k != "selections"},
        ...
        "limits": ["Every stored completed coloring and line orbit was independently rechecked on reconstructed geometry.",
                   "All failures and seven predeclared diagnostics received full saved-certificate proof replay.",
                   "Only the declared hash sample plus seven diagnostics were newly solved; this is not a second full solve.",
