"""Freeze the level/current-side candidate and replay the complete old corpus.
...
Unsupported geometric inheritance is distinct from a naming contradiction.
"""
...
from scripts.validate_level_sides import BASELINE, EXTRA_SOURCES, SOURCE, verify_run
...
def source_hashes():
    ...
    result["scripts/validate_level_sides_full.py"] = file_sha(Path(__file__))
    return result
...
def run_batch(payload):
    ...
    for saved, exported in zip(saved_rows, exports):
        ...
        require(result["status"] in ("solved", "conflict", "outside_scope"), "unaccounted status")
        if saved["key"] in six_by_key:
            archived = six_by_key[saved["key"]]
            require(json_value(result) == archived["candidate"], "six-case full certificate changed")
            require(json_value(check) == archived["candidate_check"], "six-case verification changed")
            exact += 1
...
def build_report(source_path, diagnostic_path, checkpoint_dir, workers=4, batch_size=25, limit=None, resume=False):
    ...
    report = {"schema_version": 1, "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "experiment": "Frozen mother-level/current-side rule: complete previously inspected inventory",
        ...
        "limits": ["Finite already-inspected corpus, not all plane maps or an unseen holdout.",
            ...
            "outside_scope means unrooted double-end geometric support, NOT a coloring contradiction.",
