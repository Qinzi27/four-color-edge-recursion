"""Independently recalculate the optional-level report and compare frozen v1.
...
from fourcolor.level_sides_peer import POLICY, restart_level_peer_names as restart_level_side_names
...
def replay_samples(report, six):
    ...
    # These predeclared former-unranked cases exercise a dangling line, an
    # isolated triangle and a boundary-to-boundary bent polyline.
    selected.update((
        "6124617f313f8915fdc3939d57c1f1cff1eb4ef1851be7781fae466d6110236d",
        "a9581eab845f321a9735c1ebcfefbaf2c0c489388b471c15cda4be9760d74b5e",
        "6465495161df5f8dcaf3e3cbf29ee7bea57e6db4d0167f190ddd77d71f03816d"))
    ...
    for row, export in zip(samples, exported):
        ...
        if row["key"] in six:
            def projection(value):
                """Strip descriptive-only v2 fields without producer projection code."""
                item = {k: v for k, v in json_value(value).items() if k not in ("policy", "scope")}
                item["trace"] = [{k: v for k, v in step.items() if k != "selection_group"}
                                 for step in item["trace"]]
                return item
            require(projection(fresh) == projection(six[row["key"]]["candidate"]),
                    "six-case mathematical result drift")
            require({k: v for k, v in check.items() if k != "method"} ==
                    {k: v for k, v in six[row["key"]]["candidate_check"].items() if k != "method"},
                    "six-case mathematical proof check drift")
...
def audit_v1_equivalence(report, path):
    """Check every previously rooted result, not only the six successful examples."""
    old = read_json(path)
    by_key = {r["key"]: r for r in old["drawings"]}
    require(set(by_key) == {r["key"] for r in report["drawings"]}, "v1/v2 inventory differs")
    fields = ("status", "domains", "anchors_by_dart", "colors", "choices", "backtracks",
              "old_colors_read", "local_budget", "unranked_mothers", "levels_sha256",
              "propagation_phases_sha256")
    # The full trace SHA differs because each v2 step has selection_group.
    # Exact native propagation SHA and all commitments must nonetheless match.
    rooted, former_unranked, transitions = 0, 0, Counter()
    for row in report["drawings"]:
        previous = by_key[row["key"]]
        a, b = previous["runs"][old["policy"]], row["runs"][POLICY]
        require(previous["geometry_sha256"] == row["geometry_sha256"], "v1/v2 geometry mismatch")
        transitions[(a["status"], b["status"])] += 1
        if a["status"] == "outside_scope":
            former_unranked += 1
            require(b["unranked_mothers"] == a["unranked_mothers"], "unknown mothers changed")
            require(b["levels_sha256"] == a["levels_sha256"], "unknown depth was fabricated")
        else:
            rooted += 1
            require(all(a[f] == b[f] for f in fields), "rooted behavior changed: " + row["key"])
    require(rooted + former_unranked == 7069, "v1/v2 account incomplete")
    return {"v1_filename": path.name, "v1_sha256": file_sha(path),
            "rooted_results_identical": rooted, "former_unranked_results_checked": former_unranked,
            "exact_fields_on_rooted": list(fields),
            "status_transitions": [{"v1": a, "v2": b, "count": n}
...
def main():
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "outputs/level-sides-peer-full-2026-09-19.json.gz")
    parser.add_argument("--summary", type=Path, default=ROOT / "outputs/level-sides-peer-full-summary-2026-09-19.json")
    parser.add_argument("--v1", type=Path, default=ROOT / "outputs/level-sides-full-2026-09-19.json.gz")
    ...
    for row in rows:
        ...
        require(candidate["status"] in ("solved", "conflict"), "optional level still blocked a drawing")
    ...
    v1_check = audit_v1_equivalence(report, args.v1)
    result = {"generated_at_utc": datetime.now(timezone.utc).isoformat(), "passed": True,
              ...
              "fresh_sample_replay": replay, "frozen_v1_comparison": v1_check,
